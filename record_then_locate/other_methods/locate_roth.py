#!/usr/bin/env python3
"""
locate_roth.py -- offline TDOA sound-source localization, Roth weighting.
...
Run:
    python locate_roth.py path/to/recording.wav [true_angle_deg]
"""
...
def weight(R, SIG, REF):
    """GCC-ROTH -- Roth weighting: psi = 1 / |SIG|^2, i.e. divide by the
    auto-power spectrum of the first/reference channel (SIG). Suppresses
    frequency bands where the reference channel has little energy."""
    psi = 1.0 / (np.abs(SIG) ** 2 + EPS)
    return R * psi
