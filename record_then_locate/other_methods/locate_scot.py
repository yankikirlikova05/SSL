#!/usr/bin/env python3
"""
locate_scot.py -- offline TDOA sound-source localization, Smoothed Coherence Transform.
...
Run:
    python locate_scot.py path/to/recording.wav [true_angle_deg]
"""
...
def weight(R, SIG, REF):
    ...
    psi = 1 / sqrt(|SIG|^2 * |REF|^2) = 1 / (|SIG| * |REF|).
    Whitens using both channels' auto-power spectra."""
    psi = 1.0 / (np.sqrt((np.abs(SIG) ** 2) * (np.abs(REF) ** 2)) + EPS)
    return R * psi
