#!/usr/bin/env python3
"""
locate_gtcc.py -- offline TDOA sound-source localization, generalized cubic cross-correlation (PHAT + cubic).
...
Run:
    python locate_gtcc.py path/to/recording.wav [true_angle_deg]
"""
...
def weight(R, SIG, REF):
    """GTCC -- start from a PHAT-weighted spectrum: psi = 1 / |R|.
    The distinguishing cubic step happens in postprocess() below."""
    psi = 1.0 / (np.abs(R) + EPS)
    return R * psi


def postprocess(cc):
    """GTCC -- generalized *cubic* cross-correlation: raise the time-domain
    correlation to the 3rd power preserving sign. Sharpens the true peak but is
    the higher-compute variant of the study."""
    return np.sign(cc) * np.abs(cc) ** 3
# ======================================================================
