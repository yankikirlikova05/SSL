#!/usr/bin/env python3
"""
locate_scot_phat.py -- offline TDOA sound-source localization, tunable joint SCOT-PHAT weighting.
...
Run:
    python locate_scot_phat.py path/to/recording.wav [true_angle_deg]
"""
...
# ======================================================================
# ALPHA blends SCOT (alpha=0) and PHAT (alpha=1). ADJUSTABLE module-level knob;
# default 0.7 leans toward PHAT while retaining some SCOT whitening.
ALPHA = 0.7


def weight(R, SIG, REF):
    """SCOT-PHAT -- tunable joint weighting combining SCOT and PHAT:
        psi = 1 / ( (|SIG|*|REF|)^(1-ALPHA) * |R|^ALPHA ).
    ALPHA is adjustable (see above): 0 -> pure SCOT, 1 -> pure PHAT."""
    scot_term = (np.abs(SIG) * np.abs(REF)) ** (1.0 - ALPHA)
    phat_term = (np.abs(R)) ** ALPHA
    psi = 1.0 / (scot_term * phat_term + EPS)
    return R * psi
